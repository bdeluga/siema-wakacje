...
import os
import osmnx as ox
from .models import City, Place, Opening_hours, Review, User


# City API views
def city_view(request):
    cities = City.objects.all()
    data = [{'name': city.name, 'country': city.country, 'population': city.population,
             'latitude': city.latitude, 'longitude': city.longitude} for city in cities]
...
def place_view(request):
    ...
    data = [{'name': place.name, 'description': place.description, 'address': place.address, 'image': place.image,
             'website': place.website, 'city': place.city.name, 'place_type': place.place_type} for place in places]
    return JsonResponse(data, safe=False)


def place_detail_view(request, place_id):
    place = Place.objects.get(id=place_id)
    data = {'name': place.name, 'description': place.description, 'address': place.address,
            'image': place.image, 'website': place.website, 'city': place.city.name, 'place_type': place.place_type}
    return JsonResponse(data)


def place_opening_hours_view(request, place_id):
    place = Place.objects.get(id=place_id)
    opening_hours = Opening_hours.objects.filter(place=place)
    data = [{'day_of_week': opening_hour.get_day_of_week_display(), 'open_time': opening_hour.open_time.strftime(
        '%H:%M:%S'), 'close_time': opening_hour.close_time.strftime('%H:%M:%S')} for opening_hour in opening_hours]
    return JsonResponse(data, safe=False)
...
def place_reviews_view(request, place_id):
    ...
    data = [{'user': review.user.username, 'rating': review.rating,
             'comment': review.comment} for review in reviews]
    return JsonResponse(data,  safe=False)


# Search view by city name
def search_by_city_view(request):
    ...
    for place in places:
        try:
            opening_hours = Opening_hours.objects.filter(place=place)
            reviews = Review.objects.filter(place=place)
            places_data.append({'name': place.name, 'description': place.description, 'address': place.address, 'image': place.image, 'website': place.website, 'place_type': place.place_type, 'opening_hours': [{'day_of_week': opening_hour.get_day_of_week_display(
            ), 'open_time': opening_hour.open_time.strftime('%H:%M:%S'), 'close_time': opening_hour.close_time.strftime('%H:%M:%S')} for opening_hour in opening_hours], 'reviews': [{'user': review.user.username, 'rating': review.rating, 'comment': review.comment} for review in reviews]})
        except ObjectDoesNotExist:
            return JsonResponse({"error":"Opening hours or Review not found"})
    data = {'city': city_name, 'places': places_data}
    return JsonResponse(data)
    ...
    for ind in hotelResponse.index:
        ...
        if hotelResponse['name'][ind] != hotelResponse['name'][ind]:
            ...
            if hotelResponse['geometry'][ind].geom_type == 'Point':
                ...
                place = str(hotelResponse['geometry'][ind].y) + \
                    ', ' + str(hotelResponse['geometry'][ind].x)
                # repeat request but now its concentrating to 500m around point(place)
                simpleResp = ox.geometries_from_place(
                    place, tags=tags, buffer_dist=500)
            ...
            if hotelResponse['geometry'][ind].geom_type == 'Polygon':
                ...
                simpleResp = ox.geometries_from_polygon(
                    hotelResponse['geometry'][ind], tags=tags)
